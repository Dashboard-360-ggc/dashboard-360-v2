...
// Asegúrate de que esta línea NO tenga "async"
export default function InventarioPage() {
  const [productos, setProductos] = useState<Producto[]>([]);
  const [nuevoProducto, setNuevoProducto] = useState({ nombre: '', cantidad: 0, unidad: '' });
  const [loading, setLoading] = useState(true);

  // Esta función DENTRO del componente sí puede ser async
      ...
      <form onSubmit={handleAddProducto} className="mb-6 p-4 border rounded-lg bg-gray-50">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <input
            ...
            required
          />
          <button type="submit" className="bg-blue-600 text-white p-2 rounded-md hover:bg-blue-700">Agregar</button>
        </div>
      </form>
            ...
            <tbody>
              {productos.length > 0 ? (
                productos.map((producto) => (
                  <tr key={producto.id} className="text-center hover:bg-gray-100">
                ...
                <tr>
                  <td colSpan={3} className="text-center py-4">No hay productos en el inventario.</td>
                </tr>
              )}
            </tbody>
          </table>
